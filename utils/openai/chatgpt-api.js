...
function hasAssistantFunctionTurn(message) {
    return message.role === 'assistant' && (!!message.function_call || !!message.tool_calls?.length);
}
function isFunctionTurnSequenceValid(messages, systemMessageOffset) {
...
export class ChatGPTAPI {
     ...
     */
    constructor(opts) {
        const { apiKey, apiOrg, apiBaseUrl = 'https://api.openai.com/v1', debug = false, messageStore, completionParams, systemMessage, maxModelTokens = 16000, maxResponseTokens = 8192, chatgptBlockCount = 0, getMessageById, upsertMessage, fetch = globalFetch } = opts;
        this._apiKey = apiKey;
        this._apiOrg = apiOrg;
    ...
    _toRequestMessage(message) {
        ...
        const assistantContent = storedRole === 'assistant' && (hasToolCalls || message.functionCall)
            ? (content || null)
            : (content || '');
        return {
            role: storedRole,
    ...
    async _getMessageTokenEstimate(message) {
     ...
     */
    async sendMessage(content, opts = {}, role = 'user') {
        ...
        const responseP = new Promise(async (resolve, reject) => {
            ...
            if (body.functions?.length > 0) {
                body.tools = body.functions.map((func) => ({
                    type: 'function',
                    function: func
                }));
                    ...
                    onMessage: (data) => {
                        ...
                        try {
                            ...
                            if (response.choices?.length) {
                                ...
                                if (existingUsage && !result.detail.usage) {
                                    result.detail.usage = existingUsage;
                                }
                                onProgress?.(result);
                            }
                        }
    ...
    async _buildMessages(currentMessages, opts, completionParams) {
        ...
        do {
            ...
            if (storedRole === 'tool') {
                ...
                const toolRequestMessages = toolHistoryMessages
                    .map(message => this._toRequestMessage(message))
                    .filter(Boolean);
                if (assistantRequestMessage?.role !== 'assistant' ||
